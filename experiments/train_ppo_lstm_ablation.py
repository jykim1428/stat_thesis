"""LSTM ablation: raw vs zscore vs zscore_clip (val 기준)

Run from the repository root:
    python experiments/train_ppo_lstm_ablation.py --condition raw --seed 42
    python experiments/train_ppo_lstm_ablation.py --condition zscore --seed 42
    python experiments/train_ppo_lstm_ablation.py --condition zscore_clip --seed 42
...
및 FEATURES_EXTRACTOR_CLASS만 다름 (LSTMFeaturesExtractor 사용).
"""
...
FEATURES_EXTRACTOR_KWARGS = {"hidden_size": 32, "num_layers": 2, "dropout": 0.1}
# ──────────────────────────────────────────────────────────────
